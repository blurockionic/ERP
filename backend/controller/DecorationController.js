...
export const updateDecoration = async (req, res) => {
  try {
    ...
    existingDecoration.decorationOrderedItem =
      decorationOrderedItem || existingDecoration.decorationOrderedItem;
    existingDecoration.decorationOrderedCount =
      decorationOrderedCount || existingDecoration.decorationOrderedCount;
    ...
    res
      .status(200)
      .json({
        message: "Decoration entry updated successfully",
...
//get specific decoration details
export const getSpecifivDecorationDetails = async (req, res) => {
  const { id } = req.params;
  try {
    const orders = await Decoration.findOne({ customerId: id });
    res.status(200).json({
      success: true,
      orders,
    });
